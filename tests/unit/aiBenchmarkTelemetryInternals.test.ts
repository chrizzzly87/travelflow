...
describe("ai-benchmark telemetry internals", () => {
  it("normalizes telemetry security filters and attack categories", () => {
    expect(
      __benchmarkTelemetryInternals.normalizeTelemetrySecurityFilter("blocked"),
    ).toBe("blocked");
    expect(
      __benchmarkTelemetryInternals.normalizeTelemetrySecurityFilter(
        "Suspicious",
      ),
    ).toBe("suspicious");
    expect(
      __benchmarkTelemetryInternals.normalizeTelemetrySecurityFilter("weird"),
    ).toBe("all");

    expect(
      __benchmarkTelemetryInternals.normalizeTelemetryAttackCategory(
        "prompt_exfiltration",
      ),
    ).toBe("prompt_exfiltration");
    expect(
      __benchmarkTelemetryInternals.normalizeTelemetryAttackCategory(
        "not-real",
      ),
  ...
  it("parses telemetry rows including nested security metadata", () => {
    const rows = __benchmarkTelemetryInternals.toTelemetryRows([
      {
        ...
        error_message:
          "Trip generation returned an invalid response. Please try again.",
        ...
        metadata: {
          ...
          details:
            "Provider output was rejected after generation because it failed response validation or hard trip constraints.",
          security: {
            ...
            matchedRules: [
              ...
              "missing_required_cities",
            ],
            flaggedFields: ["notes"],
            sanitization: {
              applied: true,
              changedFields: ["notes"],
              removedRuleIds: ["prompt_exfiltration_request"],
            },
    ...
    expect(rows[0]).toMatchObject({
      ...
      matched_rules: ["schema_validation_failed", "missing_required_cities"],
