...
- [x] [Improved] ⚠️ Canceled subscriptions now stay visible as a grace-period state with clear downgrade timing, resubscribe prompts, and a clearer explanation of what becomes read-only after paid access ends.
...
- [x] [Fixed] 🗓️ Profile pages now use the real account creation date for “Member since” instead of falling back to missing or misleading guesses.
...
- [x] [Improved] 🎟️ Discount links and voucher codes now stay inside checkout, apply only when you confirm them, and stay in sync with the secure payment step more cleanly.
...
- [x] [Improved] 🕵️ Admin user and audit views now surface billing lifecycle details, grace-period users, and exportable targeting data so subscription mismatches and cancellation outreach are easier to inspect.
...
- [ ] [Internal] 🔁 Added a current-user Paddle refresh endpoint so pricing, checkout, and profile settings can re-sync stale local billing state after management returns or missed webhooks.
