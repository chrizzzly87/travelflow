...
- [x] [Improved] 🔎 Calendar zoom controls now show the current scale in compact 0.2 steps, keep the controls stable while you zoom, and refit automatically when you switch calendar direction so Fit favors a tighter view over oversized white space.
...
- [ ] [Internal] 🧪 Added regression coverage for the docked-map geometry retry, planner keyboard city navigation, fit zoom controls, and the updated pane-resize behavior.
