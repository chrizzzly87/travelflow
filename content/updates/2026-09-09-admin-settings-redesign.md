...
- [ ] [Internal] 🔍 Added a shared search field built on the existing input-group primitive, so the icon is a flex sibling of the input instead of an absolutely positioned overlay that drifted whenever a gutter changed. The model picker now uses it.
- [ ] [Internal] 📌 The model picker keeps the chosen model visible in a pinned dialog footer while you scroll the catalogue, and commits it with an explicit button — picking a row no longer applies and closes in one step, so a mis-click is recoverable with Cancel.
- [ ] [Internal] 🧰 Added the new shared components to the admin Design Playground: a Settings Panels tab, the search field and stepper input under Inputs, alert tones, and a dialog sample that actually scrolls so the pinned footer is visible.
