...
- [x] [Fixed] 🚪 Fast clicks into create-trip and profile now wait for the real route handoff instead of briefly repainting the loading shell during auth or page-chunk setup.
...
- [ ] [Internal] 🪶 Removed redundant nested suspense wrappers around create-trip route pages so route transitions no longer force a second fallback paint after navigation starts.
- [ ] [Internal] 🖱️ Re-enabled pointer and touch route warmups on first-load-critical pages while keeping passive hover and viewport prefetch suppressed there.
- [ ] [Internal] 🧪 Restored navigation prefetch on localhost by default so debugging and local route-response checks match production behavior more closely.
