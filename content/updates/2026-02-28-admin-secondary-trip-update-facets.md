...
- [ ] [Improved] 📊 Admin audit now shows a timeline-diff migration readiness indicator (v1 vs legacy-only rows in the loaded window).
...
- [ ] [Internal] 🧭 Lifecycle trip updates now write typed secondary facets/sub-events for settings, visibility, and trip-date changes so audit rows stay deterministic outside version commits.
- [ ] [Internal] 🧭 Deleted timeline gap segments now emit explicit `trip.segment.deleted` secondary/domain events for deterministic audit facet coverage.
