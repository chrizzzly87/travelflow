...
export const AdminDestinationsPage: React.FC = () => {
    ...
    return (
        <AdminShell
            ...
            showDateRange={false}
            actions={(
                <button type="button" onClick={() => void loadCatalog()} disabled={loading} className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-3 py-2 text-sm font-semibold text-foreground hover:bg-secondary disabled:opacity-50">
                ...
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    {[
                        ...
                        <AdminSurfaceCard key={String(label)} className="flex items-center justify-between">
                            <div><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{String(label)}</div><div className="mt-1 text-2xl font-bold text-foreground">{String(value)}</div></div>
                            {React.createElement(Icon as React.ComponentType<{ size: number }>, { size: 24 })}
                        </AdminSurfaceCard>
                    ))}
                </div>

                {latestRun && <div className="rounded-xl border border-border bg-secondary px-4 py-3 text-sm text-muted-foreground">Latest import: <strong>{String(latestRun.provider || 'unknown')}</strong> · {String(latestRun.status || 'unknown')} · {formatDate(latestRun.completed_at || latestRun.started_at)}</div>}
                {error && <div className="flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:bg-red-400/12 dark:text-red-200 dark:border-red-400/30"><WarningCircle size={18} className="mt-0.5 shrink-0" />{error}</div>}
                ...
                <div className="grid min-h-[640px] gap-4 xl:grid-cols-[minmax(380px,0.9fr)_minmax(520px,1.1fr)]">
                    <AdminSurfaceCard className="flex min-h-0 flex-col p-0">
                        ...
                        <div className="max-h-[720px] flex-1 overflow-y-auto p-2">
                            ...
                            {!loading && view === 'guides' && filteredGuides.map((guide) => {
                                ...
                                return <button key={guide.id} type="button" onClick={() => selectItem({ targetKind: 'guide', targetId: guide.id }, existing)} className={cn('mb-1 flex w-full items-center justify-between rounded-xl border px-3 py-3 text-start transition-colors', active ? 'border-accent-300 bg-accent-50 dark:bg-accent-400/12 dark:border-accent-400/30' : 'border-transparent hover:border-border hover:bg-secondary')}><div className="min-w-0"><div className="truncate font-semibold text-foreground">{guide.name}</div><div className="mt-0.5 text-xs uppercase tracking-wide text-muted-foreground">{guide.kind} · {guide.country_code} · {guide.region}</div></div><span className={cn('rounded-full px-2 py-1 text-[11px] font-semibold', existing?.status === 'published' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-400/12 dark:text-emerald-200' : existing ? 'bg-amber-100 text-amber-800 dark:bg-amber-400/12 dark:text-amber-200' : 'bg-secondary text-muted-foreground')}>{existing?.status || 'base'}</span></button>;
                            })}
                            {!loading && view === 'profiles' && filteredProfiles.map((profile) => {
                                ...
                                return <button key={profile.country_code} type="button" onClick={() => selectItem({ targetKind: 'country_profile', targetId: profile.country_code }, existing)} className={cn('mb-1 flex w-full items-center justify-between rounded-xl border px-3 py-3 text-start transition-colors', active ? 'border-accent-300 bg-accent-50 dark:bg-accent-400/12 dark:border-accent-400/30' : 'border-transparent hover:border-border hover:bg-secondary')}><div className="min-w-0"><div className="truncate font-semibold text-foreground">{profile.name}</div><div className="mt-0.5 text-xs uppercase tracking-wide text-muted-foreground">{profile.country_code} · {profile.region} · {profile.source_provider}</div></div><span className={cn('rounded-full px-2 py-1 text-[11px] font-semibold', existing?.status === 'published' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-400/12 dark:text-emerald-200' : existing ? 'bg-amber-100 text-amber-800 dark:bg-amber-400/12 dark:text-amber-200' : 'bg-secondary text-muted-foreground')}>{existing?.status || 'base'}</span></button>;
                            })}
                        </div>
                    </AdminSurfaceCard>
                    ...
                    <AdminSurfaceCard className="min-w-0">
                        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border pb-4">
                            <div><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Editorial override</div><h2 className="mt-1 text-xl font-bold text-foreground">{selectedName}</h2>{selectedOverride && <div className="mt-1 text-xs text-muted-foreground">Updated {formatDate(selectedOverride.updated_at)}</div>}</div>
                            {publicPath && <a href={publicPath} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg border border-border px-3 py-2 text-sm font-semibold text-foreground hover:bg-secondary">Open page <ArrowSquareOut size={15} /></a>}
                        </div>
                        {selection ? <div className="mt-4 space-y-4">
                            {selectedProfile && <div className="rounded-xl border border-border bg-secondary p-3 text-sm"><div className="font-semibold text-foreground">Source provenance</div><a className="mt-1 block break-all text-accent-700 hover:underline dark:text-accent-200" href={selectedProfile.origin_url} target="_blank" rel="noreferrer">{selectedProfile.origin_url}</a><div className="mt-1 text-xs text-muted-foreground">Fetched {formatDate(selectedProfile.source_fetched_at)}</div></div>}
                            <div className="grid gap-3 sm:grid-cols-2"><label className="space-y-1 text-sm font-semibold text-foreground">Status<Select value={editorStatus} onValueChange={(value) => setEditorStatus(value as AdminDestinationOverrideStatus)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="draft">Draft — admin only</SelectItem><SelectItem value="published">Published — public</SelectItem></SelectContent></Select></label><label className="space-y-1 text-sm font-semibold text-foreground">Editor note<Input value={editorNote} onChange={(event) => setEditorNote(event.target.value)} placeholder="Why is this changed?" /></label></div>
                            <label className="block space-y-1 text-sm font-semibold text-foreground">Override patch<textarea value={editorJson} onChange={(event) => setEditorJson(event.target.value)} spellCheck={false} className="min-h-64 w-full resize-y rounded-xl border border-border bg-slate-950 p-4 font-mono text-xs font-normal leading-5 text-slate-100 outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-100" aria-describedby="destination-override-help" /></label>
                            <p id="destination-override-help" className="text-xs leading-5 text-muted-foreground">Only include fields you want to change. Nested objects merge; arrays replace the complete source array. Identity and provenance fields cannot be overridden.</p>
                            <div className="flex flex-wrap gap-2"><button type="button" onClick={() => void save()} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-accent-600 px-4 py-2 text-sm font-semibold text-white hover:bg-accent-700 disabled:opacity-50">{saving ? <SpinnerGap size={17} className="animate-spin" /> : <FloppyDisk size={17} />} {editorStatus === 'published' ? 'Publish override' : 'Save draft'}</button>{selectedOverride && <button type="button" onClick={() => void reset()} disabled={saving} className="inline-flex items-center gap-2 rounded-xl border border-red-200 px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-50 disabled:opacity-50 dark:hover:bg-red-400/12 dark:text-red-200 dark:border-red-400/30"><Trash size={17} /> Reset override</button>}</div>
                            <details className="rounded-xl border border-border"><summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-foreground">Effective content preview</summary><pre className="max-h-80 overflow-auto border-t border-border bg-secondary p-4 text-xs leading-5 text-foreground">{JSON.stringify(effectivePreview, null, 2)}</pre></details>
                        </div> : <div className="flex h-64 items-center justify-center text-sm text-muted-foreground">Select a destination item to inspect it.</div>}
                    </AdminSurfaceCard>
                </div>
            </div>
        </AdminShell>
