...
interface AiTelemetryFailureSummary {
  totalFailed: number;
  blockedInput: number;
...
interface AiTelemetryApiResponse extends BenchmarkApiResponse {
  ...
  failureSummary?: AiTelemetryFailureSummary;
}
...
const TELEMETRY_SECURITY_FILTER_OPTIONS: Array<{
  value: TelemetrySecurityFilter;
  label: string;
}> = [
...
const formatFailureBucketLabel = (
  value?: AiTelemetryRecentRow["failure_bucket"],
): string => {
  switch (value) {
    case "blocked_input":
      return "Blocked before provider";
    case "blocked_output":
      return "Blocked after provider";
    case "invalid_output":
      return "Invalid output";
    case "user_aborted":
      return "User aborted retry";
    case "provider_failed":
      return "Provider failed";
    case "other_failed":
      return "Other failed attempt";
    default:
      return "—";
  }
};
...
const createProviderChartTooltip = (
  ...
  return ({ active, payload, label }) => {
    ...
    const firstPayload = payload[0]?.payload as
      ...
      | undefined;
    const providerId =
      typeof firstPayload?.providerId === "string"
    ...
    return (
      <div className="rounded-md border border-slate-200 bg-white px-2 py-1.5 shadow-lg">
        <p className="mb-1 text-[11px] font-semibold text-slate-900">
          <ProviderLabel
            provider={providerId}
            providerClassName="text-slate-900"
            logoSize={12}
          />
        </p>
        <div className="space-y-0.5">
          {payload.map((entry) => {
            ...
            const metricKey =
              typeof entry.dataKey === "string" && entry.dataKey.trim().length > 0
                ? entry.dataKey
                : metricName;
            return (
              <p
...
const ProviderBreakdownTooltip = createProviderChartTooltip((_name, value) =>
  formatCallCount(value),
);
const ProviderSuccessRateTooltip = createProviderChartTooltip((_name, value) =>
  formatPercent(value),
);
const ProviderCostPerSuccessTooltip = createProviderChartTooltip(
  (_name, value) => formatUsd(value),
);
...
const ProviderLabel: React.FC<{
  ...
  return (
    <span className="inline-flex min-w-0 items-center gap-1.5">
      ...
      <span
        className={providerClassName || "font-semibold text-slate-800"}
        title={`${metadata.label} (${provider})`}
      >
        {metadata.shortName}
      </span>
      {model ? (
  ...
  return (
    <span className="inline-flex min-w-0 max-w-[270px] items-center gap-1.5">
      ...
      <span
        ...
        title={`${metadata.shortName} / ${model}`}
      >
...
export const AdminAiTelemetryPage: React.FC = () => {
  ...
  const [telemetryWindowHours, setTelemetryWindowHours] = useState<number>(
    24 * 7,
  );
  const [telemetrySource, setTelemetrySource] =
    useState<TelemetrySourceFilter>("all");
  const [telemetryProviderFilter, setTelemetryProviderFilter] =
    useState<string>("all");
  const [telemetrySecurityFilter, setTelemetrySecurityFilter] =
    useState<TelemetrySecurityFilter>("all");
  const [telemetryAttackCategoryFilter, setTelemetryAttackCategoryFilter] =
    useState<string>("all");
  ...
  const [telemetrySummary, setTelemetrySummary] =
    useState<AiTelemetrySummary | null>(null);
  const [telemetrySeries, setTelemetrySeries] = useState<
    AiTelemetrySeriesPoint[]
  >([]);
  const [monthlyCostSeries, setMonthlyCostSeries] = useState<
    Array<{ date: string; cost: number }>
  >([]);
  const [telemetryProviders, setTelemetryProviders] = useState<
    AiTelemetryProviderPoint[]
  >([]);
  const [telemetryModels, setTelemetryModels] = useState<
    AiTelemetryModelPoint[]
  >([]);
  const [telemetryRecent, setTelemetryRecent] = useState<
    AiTelemetryRecentRow[]
  >([]);
  const [telemetryRecentIncidents, setTelemetryRecentIncidents] = useState<
    AiTelemetryRecentRow[]
  >([]);
  const [telemetrySecuritySummary, setTelemetrySecuritySummary] =
    useState<AiTelemetrySecuritySummary | null>(null);
  const [telemetryFailureSummary, setTelemetryFailureSummary] =
    useState<AiTelemetryFailureSummary | null>(null);
  const [telemetryCommentTotal, setTelemetryCommentTotal] = useState(0);
  const [telemetryCommentGroups, setTelemetryCommentGroups] = useState<
    AiTelemetryRunCommentGroup[]
  >([]);
  const [telemetryProviderOptions, setTelemetryProviderOptions] = useState<
    string[]
  >([]);
  const [telemetryAttackCategoryOptions, setTelemetryAttackCategoryOptions] =
    useState<string[]>([]);
  ...
  const [bestValueModels, setBestValueModels] = useState<
    AiTelemetryModelPoint[]
  >([]);
  const [selectedProviderShare, setSelectedProviderShare] = useState<
    string | null
  >(null);
  const [selectedModelShare, setSelectedModelShare] = useState<string | null>(
    null,
  );
  ...
  const fetchTelemetryApi = useCallback(
    async (path: string, init?: RequestInit): Promise<BenchmarkApiResponse> => {
      if (!accessToken) {
        throw new Error(
          "No Supabase session access token found. Refresh and try again.",
        );
      }
      ...
      try {
        payload = (
          raw ? JSON.parse(raw) : { ok: response.ok }
        ) as BenchmarkApiResponse;
      } catch {
      ...
      if (!response.ok) {
        throw new Error(
          payload.error || `Telemetry API request failed (${response.status})`,
        );
      }
  ...
  const loadTelemetry = useCallback(async () => {
    ...
    try {
      ...
      const payload = (await fetchTelemetryApi(
        `/api/internal/ai/benchmark/telemetry?${params.toString()}`,
        {
          method: "GET",
        },
      ...
      setMonthlyCostSeries(
        Array.isArray(payload.monthlyCostSeries)
          ? payload.monthlyCostSeries
          : [],
      );
      ...
      setTelemetryRecentIncidents(
        Array.isArray(payload.recentIncidents) ? payload.recentIncidents : [],
      );
      setTelemetrySecuritySummary(payload.securitySummary || null);
      setTelemetryFailureSummary(payload.failureSummary || null);
      setTelemetryCommentTotal(Number(payload.comments?.total) || 0);
      setTelemetryCommentGroups(
        Array.isArray(payload.comments?.groups) ? payload.comments?.groups : [],
      );
      setFastestModels(
        Array.isArray(payload.rankings?.fastest)
          ? payload.rankings?.fastest
          : [],
      );
      setCheapestModels(
        Array.isArray(payload.rankings?.cheapest)
          ? payload.rankings?.cheapest
          : [],
      );
      setBestValueModels(
        Array.isArray(payload.rankings?.bestValue)
          ? payload.rankings?.bestValue
          : [],
      );

      const providerOptions = Array.isArray(payload.availableProviders)
        ? [...payload.availableProviders]
        : [];
      providerOptions.sort((left, right) => {
        const orderDelta =
          getAiProviderSortOrder(left) - getAiProviderSortOrder(right);
        if (orderDelta !== 0) return orderDelta;
        return getAiProviderMetadata(left).shortName.localeCompare(
          getAiProviderMetadata(right).shortName,
        );
      });
      setTelemetryProviderOptions(providerOptions);
      const attackCategoryOptions = Array.isArray(
        payload.availableAttackCategories,
      )
        ? [...payload.availableAttackCategories].sort((left, right) =>
            left.localeCompare(right),
          )
        : [];
      setTelemetryAttackCategoryOptions(attackCategoryOptions);
      if (
        telemetryProviderFilter !== "all" &&
        !providerOptions.includes(telemetryProviderFilter)
      ) {
        setTelemetryProviderFilter("all");
      }
      if (
        telemetryAttackCategoryFilter !== "all" &&
        !attackCategoryOptions.includes(telemetryAttackCategoryFilter)
      ) {
        setTelemetryAttackCategoryFilter("all");
      }
    } catch (telemetryLoadError) {
      setTelemetryError(
        telemetryLoadError instanceof Error
          ? telemetryLoadError.message
          : "Failed to load AI telemetry",
      );
      ...
      setTelemetryFailureSummary(null);
  ...
  useEffect(() => {
    ...
    const providerStillVisible = telemetryProviders.some(
      (row) => row.provider === selectedProviderShare,
    );
  ...
  const successRateLeaders = useMemo(() => {
    return [...successfulModels]
      .sort(
        (left, right) =>
          right.successRate - left.successRate || right.total - left.total,
      )
      .slice(0, rankLimit);
  ...
  const successRateTrendChartData = useMemo(() => {
    return telemetrySeries.map((point) => ({
      ...
      "Success rate":
        point.total > 0
          ? Number(((point.success / point.total) * 100).toFixed(1))
          : 0,
    }));
  }, [telemetrySeries]);
  ...
  const providerModelDonutEntries = useMemo(() => {
    if (!selectedProviderShare) return [];
    return buildProviderModelDonutEntries(
      telemetryModels,
  ...
  const activeDonutEntries = selectedProviderShare
    ? providerModelDonutEntries
    : providerDonutEntries;

  useEffect(() => {
    if (!selectedProviderShare || !selectedModelShare) return;
    const modelStillVisible = providerModelDonutEntries.some(
      (entry) => entry.model === selectedModelShare,
    );
    ...
    return activeDonutEntries.map((entry) => ({
      Segment: selectedProviderShare
        ...
        : entry.provider === "other"
          ? entry.label
          : getAiProviderMetadata(entry.provider).shortName,
      Calls: entry.calls,
  ...
  const providerCostPerSuccessChartData = useMemo(
    () =>
      buildProviderCostPerSuccessChartData(telemetryProviders, 8).map(
        (row) => ({
          providerId: row.Provider,
          ...row,
          Provider: getAiProviderMetadata(row.Provider).shortName,
  ...
  const selectedModelStats = useMemo(() => {
    ...
    return (
      telemetryModels.find(
        (row) =>
          row.provider === selectedProviderShare &&
          row.model === selectedModelShare,
      ) || null
    );
  }, [selectedModelShare, selectedProviderShare, telemetryModels]);

  const handleProviderShareSliceSelect = useCallback(
    (value: unknown) => {
  ...
  const successfulModelCoverage = useMemo(() => {
    ...
    return Number(
      ((successfulModels.length / telemetryModels.length) * 100).toFixed(1),
    );
  ...
  const suspiciousRate = useMemo(() => {
    if (
      !telemetrySummary ||
      telemetrySummary.total <= 0 ||
      !telemetrySecuritySummary
    )
      return null;
    return Number(
      (
        (telemetrySecuritySummary.suspicious / telemetrySummary.total) *
        100
      ).toFixed(1),
    );
    ...
    <AdminShell
      ...
      actions={
        ...
        >
          <ArrowClockwise
            size={14}
            className={telemetryLoading ? "animate-spin" : ""}
          />
          Refresh
        </button>
      }
    >
      <div className="mx-auto w-full max-w-[1600px] space-y-4">
        ...
        <AdminSurfaceCard>
          <Title>Filters</Title>
          <Subtitle>
            Keep this compact up top, then scroll down for deeper telemetry
            analysis.
          </Subtitle>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-7">
            <div className="space-y-1">
              ...
              <Select
                value={String(telemetryWindowHours)}
                onValueChange={(value) =>
                  setTelemetryWindowHours(Number(value))
                }
              >
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setTelemetrySource(value as TelemetrySourceFilter)
                }
              >
                <SelectTrigger className="h-9 text-sm">
            ...
            <div className="space-y-1">
              ...
              <Select
                value={telemetryProviderFilter}
                onValueChange={setTelemetryProviderFilter}
              >
                <SelectTrigger className="h-9 text-sm">
            ...
            <div className="space-y-1">
              ...
              <Select
                value={telemetrySecurityFilter}
                onValueChange={(value) =>
                  setTelemetrySecurityFilter(value as TelemetrySecurityFilter)
                }
              >
                <SelectTrigger className="h-9 text-sm">
            ...
            <div className="space-y-1">
              ...
              <Select
                value={telemetryAttackCategoryFilter}
                onValueChange={setTelemetryAttackCategoryFilter}
              >
                <SelectTrigger className="h-9 text-sm">
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(value) => setRankLimit(Number(value))}
              >
                <SelectTrigger className="h-9 text-sm">
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <AdminSurfaceCard>
            ...
            <Metric>
              {telemetrySummary
                ? formatPercent(telemetrySummary.successRate)
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
              Failed: {telemetrySummary ? telemetrySummary.failed : "—"}
            </Text>
          </AdminSurfaceCard>
          ...
          <AdminSurfaceCard>
            ...
            <Metric>
              {telemetrySummary
                ? formatDuration(telemetrySummary.averageLatencyMs)
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
              Across all calls
          ...
          <AdminSurfaceCard>
            ...
            <Metric>
              {telemetrySummary
                ? formatUsd(telemetrySummary.totalCostUsd)
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <AdminSurfaceCard>
            ...
            <Metric>
              {telemetrySecuritySummary
                ? telemetrySecuritySummary.suspicious
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
          ...
          <AdminSurfaceCard>
            ...
            <Metric>
              {telemetrySecuritySummary
                ? telemetrySecuritySummary.blocked
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
              High-confidence runtime guard blocks
            </Text>
          </AdminSurfaceCard>
          ...
          <AdminSurfaceCard>
            ...
            <Metric>
              {telemetrySecuritySummary
                ? formatPercent(telemetrySecuritySummary.blockRate)
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
              Based on the current telemetry filter scope
            </Text>
          </AdminSurfaceCard>
          ...
          <AdminSurfaceCard>
            ...
            <Metric className="text-[1.5rem] leading-tight">
              {topAttackType
                ? formatAttackCategoryLabel(topAttackType.category)
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
              {topAttackType
                ? `${topAttackType.count.toLocaleString()} matched incidents`
                : "No suspicious incidents in scope"}
            </Text>
          </AdminSurfaceCard>
        </section>
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          <AdminSurfaceCard>
            <Text>Blocked before provider</Text>
            <Metric>
              {telemetryFailureSummary
                ? telemetryFailureSummary.blockedInput
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
              High-confidence input preflight blocks
            </Text>
          </AdminSurfaceCard>

          <AdminSurfaceCard>
            <Text>Blocked after provider</Text>
            <Metric>
              {telemetryFailureSummary
                ? telemetryFailureSummary.blockedOutput
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
              Model returned output that the guard rejected
            </Text>
          </AdminSurfaceCard>

          <AdminSurfaceCard>
            <Text>Invalid output</Text>
            <Metric>
              {telemetryFailureSummary
                ? telemetryFailureSummary.invalidOutput
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
              Failed JSON or contract validation without a guard block
            </Text>
          </AdminSurfaceCard>

          <AdminSurfaceCard>
            <Text>User-aborted retries</Text>
            <Metric>
              {telemetryFailureSummary
                ? telemetryFailureSummary.userAborted
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
              Attempts cancelled by the traveler or admin
            </Text>
          </AdminSurfaceCard>

          <AdminSurfaceCard>
            <Text>Provider failed</Text>
            <Metric>
              {telemetryFailureSummary
                ? telemetryFailureSummary.providerFailed
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
              Provider reached, but generation did not recover cleanly
            </Text>
          </AdminSurfaceCard>

          <AdminSurfaceCard>
            <Text>Other failed attempts</Text>
            <Metric>
              {telemetryFailureSummary
                ? telemetryFailureSummary.otherFailed
                : "—"}
            </Metric>
            <Text className="mt-1 text-xs text-slate-500">
              Residual failures outside the main buckets
            </Text>
          </AdminSurfaceCard>
        </section>
        ...
        <section className="grid gap-3 lg:grid-cols-2">
          <AdminSurfaceCard>
            ...
            <Subtitle>
              Success vs failed volume across the selected window.
            </Subtitle>
            {callsTrendChartData.length > 0 ? (
              ...
              <Text className="mt-3 text-xs text-slate-500">
                No time-series call data for this filter set.
              </Text>
            )}
          </AdminSurfaceCard>
          ...
          <AdminSurfaceCard>
            ...
            {successRateTrendChartData.length > 0 ? (
              ...
              <Text className="mt-3 text-xs text-slate-500">
                No success-rate trend data for this filter set.
              </Text>
            )}
          </AdminSurfaceCard>
        </section>
        ...
        <section className="grid gap-3 lg:grid-cols-2">
          <AdminSurfaceCard>
            ...
            {latencyTrendChartData.length > 0 ? (
              ...
              <Text className="mt-3 text-xs text-slate-500">
                No latency trend data for this filter set.
              </Text>
            )}
          </AdminSurfaceCard>
          ...
          <AdminSurfaceCard>
            ...
            {costTrendChartData.length > 0 ? (
              ...
              <Text className="mt-3 text-xs text-slate-500">
                No cost trend data for this filter set.
              </Text>
            )}
          </AdminSurfaceCard>
        </section>
        ...
        <section className="mt-3">
          <AdminSurfaceCard>
            ...
            <Subtitle>
              Daily breakdown of API costs for the ongoing calendar month.
            </Subtitle>
        ...
        <section className="grid gap-3 lg:grid-cols-2 mt-3">
          <AdminSurfaceCard>
            ...
            {providerVolumeChartData.length > 0 ? (
              ...
              <Text className="mt-3 text-xs text-slate-500">
                No provider breakdown in this filter set.
              </Text>
            )}
          </AdminSurfaceCard>
            ...
            {providerShareDonutData.length > 0 ? (
              <div className="mt-3 grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,250px)]">
                <DonutChart
                  ...
                  label={
                    selectedProviderShare
                      ? `${getAiProviderMetadata(selectedProviderShare).shortName} models`
                      : "Providers"
                  }
                  valueFormatter={(value) => formatCallCount(value)}
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1.5">
                    {activeDonutEntries.map((entry) => {
                      ...
                      const isDisabled =
                        !selectedProviderShare && entry.provider === "other";
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setSelectedModelShare((current) =>
                              current === entry.model ? null : entry.model,
                            );
                          }}
              ...
              <Text className="mt-3 text-xs text-slate-500">
                No provider-share data in this filter set.
              </Text>
            )}
            ...
            {selectedProviderStats && (
              <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700">
                <p className="font-semibold text-slate-900">
                  Provider details:{" "}
                  <ProviderLabel
                    provider={selectedProviderStats.provider}
                    providerClassName="font-bold text-slate-900"
                    logoSize={12}
                  />
                </p>
                <p className="mt-1">
                  Calls {selectedProviderStats.total.toLocaleString()} • Success{" "}
                  {formatPercent(
                    selectedProviderStats.total > 0
                      ? (selectedProviderStats.success /
                          selectedProviderStats.total) *
                          100
                      : null,
                  )}{" "}
                  • Avg duration{" "}
                  {formatDuration(selectedProviderStats.averageLatencyMs)} •
                  Total cost {formatUsd(selectedProviderStats.totalCostUsd)}
                </p>
              </div>
            )}
            ...
            {selectedModelStats && (
              <div className="mt-2 rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-900">
                <p className="font-semibold">
                  Model details:{" "}
                  <ProviderLabel
                    provider={selectedModelStats.provider}
                    model={selectedModelStats.model}
                    providerClassName="text-blue-900"
                    modelClassName="text-blue-800"
                    logoSize={12}
                  />
                </p>
                <p className="mt-1">
                  Calls {selectedModelStats.total.toLocaleString()} • Success{" "}
                  {formatPercent(selectedModelStats.successRate)} • Avg duration{" "}
                  {formatDuration(selectedModelStats.averageLatencyMs)} • Avg
                  cost {formatUsd(selectedModelStats.averageCostUsd)}
                </p>
              </div>
            )}
          </AdminSurfaceCard>
        ...
        <section className="grid gap-3 lg:grid-cols-3">
          <AdminSurfaceCard>
            ...
            {providerSuccessRateChartData.length > 0 ? (
              ...
              <Text className="mt-3 text-xs text-slate-500">
                No provider success-rate data available.
              </Text>
            )}
          </AdminSurfaceCard>
          ...
          <AdminSurfaceCard>
            ...
            {providerCostPerSuccessChartData.length > 0 ? (
              ...
              <Text className="mt-3 text-xs text-slate-500">
                No provider cost-per-success data available.
              </Text>
            )}
          </AdminSurfaceCard>
          ...
          <AdminSurfaceCard>
            ...
            {failureCodeBarListData.length > 0 ? (
              ...
              <Text className="mt-3 text-xs text-slate-500">
                No recent failures in the selected filter scope.
              </Text>
            )}
          </AdminSurfaceCard>
        </section>
        ...
        <section>
          <AdminSurfaceCard>
            ...
            <Subtitle>
              Suspicious or blocked runtime rows with redacted evidence and deep
              links into trip diagnostics.
            </Subtitle>
              ...
              <table className="min-w-full border-collapse text-left text-xs">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50 uppercase tracking-wide text-slate-500">
                    ...
                    <th className="px-2 py-1.5">Flagged fields</th>
                    <th className="px-2 py-1.5">Matched rules</th>
                    <th className="px-2 py-1.5">Provider reached</th>
                ...
                <tbody>
                  {securityIncidentRows.length === 0 && (
                    <tr>
                      ...
                      >
                        {telemetryLoading
                          ? "Loading incidents..."
                          : "No suspicious incidents in the selected filter scope."}
                      </td>
                    </tr>
                  )}
                  ...
                  {securityIncidentRows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-2 py-1.5 text-slate-700">
                        {formatFailureBucketLabel(row.failure_bucket)}
                      </td>
                      <td className="px-2 py-1.5 text-slate-700">
                        {row.attack_categories &&
                        row.attack_categories.length > 0
                          ? row.attack_categories
                              .map((category) =>
                                formatAttackCategoryLabel(category),
                              )
                      ...
                      <td className="px-2 py-1.5 text-slate-700">
                        {row.flagged_fields && row.flagged_fields.length > 0
                          ? row.flagged_fields.join(", ")
                          : "—"}
                      </td>
                      <td className="px-2 py-1.5 text-slate-700">
                      ...
                      <td className="px-2 py-1.5 text-slate-700">
                        {typeof row.provider_reached === "boolean"
                          ? row.provider_reached
                            ? "yes"
                            : "no"
                          : "—"}
                      </td>
        ...
        <section className="grid gap-3 lg:grid-cols-2">
          <AdminSurfaceCard>
            ...
            {modelVolumeBarListData.length > 0 ? (
              ...
              <Text className="mt-3 text-xs text-slate-500">
                No model activity data in this filter set.
              </Text>
            )}
          </AdminSurfaceCard>
          ...
          <AdminSurfaceCard>
            ...
            <Subtitle>
              Latest rows with status, duration, and error code.
            </Subtitle>
        ...
        <section>
          <AdminSurfaceCard>
            ...
            <Subtitle>
              Qualitative notes captured per run, grouped by provider/model in
              the current telemetry filter window.
            </Subtitle>
            <Text className="mt-1 text-xs text-slate-500">
            ...
            <div className="mt-3 max-h-[500px] space-y-3 overflow-y-auto rounded-xl border border-slate-200 bg-slate-50 p-3">
              {telemetryCommentGroups.length === 0 && (
                <div className="rounded-md border border-slate-200 bg-white px-3 py-4 text-xs text-slate-500">
                  {telemetryLoading
                    ? "Loading comments..."
                    : "No benchmark run comments in this filter window."}
                </div>
              )}
                ...
                >
                  ...
                  <div className="mt-2 space-y-2">
                    {group.comments.map((comment) => (
                      <div
                        key={`${group.key}-${comment.runId}-${comment.updatedAt}`}
                      ...
                      >
                        ...
                        <p className="mt-1 text-[10px] text-slate-500">
                          ...
                          {comment.satisfactionRating || "unrated"} •{" "}
