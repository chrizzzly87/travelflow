        ...
        <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-secondary">
                <tr>
                    ...
                    <th scope="col" className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-widest text-muted-foreground">Purpose</th>
                    <th scope="col" className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-widest text-muted-foreground">Duration</th>
                ...
                <section className="rounded-3xl border border-border bg-card p-6 shadow-sm md:p-8 dark:shadow-none">
                    ...
                    <div className="mt-4 flex flex-wrap gap-3">
                        <button
                            ...
                            className="rounded-lg border border-border px-4 py-2 text-sm font-semibold text-foreground hover:border-slate-400"
                    ...
                    <section key={category} className="rounded-3xl border border-border bg-card p-6 shadow-sm md:p-8 dark:shadow-none">
                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-foreground">{COOKIE_CATEGORY_COPY[category].description}</p>
                            </div>
                        </div>
                        {cookies.length === 0 ? (
                            <p className="mt-4 text-sm text-muted-foreground">No cookies are set in this category.</p>
                        ) : (
                            <Table>
                                {cookies.map((cookie) => (
                                    <tr key={cookie.name}>
                                        <td className="px-4 py-3 font-medium text-foreground">
                                            <div>{cookie.name}</div>
                                            <div className="text-xs font-normal text-muted-foreground">{cookie.storage ?? 'cookie'}</div>
                                        </td>
                                        <td className="px-4 py-3">
                                            {cookie.purpose}
                                            {cookie.notes && (
                                                <div className="mt-1 text-xs text-muted-foreground">{cookie.notes}</div>
                                            )}
                                        </td>
