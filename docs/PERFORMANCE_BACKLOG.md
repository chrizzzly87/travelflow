...
- [x] Serve `/api/trip-map-preview` as CDN-cached image bytes instead of a redirect to the provider, so a trip card's map is rendered once per trip state rather than once per visitor (`docs/TRIP_MAP_PREVIEW_CACHING.md`).
