...
export type AiRuntimeSecurityAttackCategory = (typeof AI_RUNTIME_SECURITY_ATTACK_CATEGORIES)[number];
...
export type AiRuntimeSecurityGuardDecision = (typeof AI_RUNTIME_SECURITY_GUARD_DECISIONS)[number];
...
export type AiRuntimeSecurityStage = (typeof AI_RUNTIME_SECURITY_STAGES)[number];
...
const HIGH_CONFIDENCE_BLOCK_CATEGORIES = new Set<AiRuntimeSecurityAttackCategory>([
  "prompt_exfiltration",
  "secret_exfiltration",
]);
...
const INPUT_RULES: DetectionRule[] = [
  {
    ...
    weight: 45,
    regex: /ignore(?: all| any| the)? (?:previous|prior|above|system|developer)? instructions?/i,
  },
  {
    id: "override_policy",
    category: "instruction_override",
    weight: 45,
    regex: /(?:override|bypass|disable).{0,20}(?:policy|guard(?:rail)?|safety|instruction|system)/i,
  },
  {
    ...
    weight: 80,
    forceBlock: true,
    regex: /(?:reveal|show|print|dump|quote|leak|expose|return).{0,40}(?:system|developer|hidden|internal).{0,20}(?:prompt|instruction|message|schema)/i,
  },
  {
    ...
    weight: 90,
    forceBlock: true,
    regex: /(?:reveal|show|print|dump|return|include|output).{0,50}(?:api[_ -]?key|secret|token|password|service[_ -]?role|env(?:ironment)?(?: variables?)?)/i,
  },
  {
    ...
    weight: 60,
    forceBlock: true,
    regex: /(?:plain text|markdown|free text).{0,30}(?:instead of|over|rather than).{0,20}(?:json|schema)|(?:do not|don't|no need to).{0,20}(?:use|return).{0,20}json|ignore.{0,20}(?:schema|json contract)/i,
  },
  {
    id: "constraint_override_request",
    category: "constraint_override",
    weight: 55,
    regex: /(?:ignore|change|reorder|drop|add).{0,40}(?:required cities|requested cities|route lock|locked order|city order|constraints?)/i,
  },
  {
  ...
  },
  {
    ...
    category: "suspicious_destination_payload",
    weight: 25,
    regex: /<\s*(?:system|assistant|developer|instructions?|prompt|tool)[^>]*>/i,
  },
];
...
const OUTPUT_RULES: DetectionRule[] = [
  {
    ...
    weight: 90,
    forceBlock: true,
    regex: /(?:system prompt|developer message|hidden instructions|internal prompt|prompt template|json schema|required fields|additionalProperties)/i,
  },
  {
    ...
    weight: 100,
    forceBlock: true,
    regex: /\b(?:sk-[a-zA-Z0-9]{10,}|AIza[0-9A-Za-z\-_]{20,}|SUPABASE_SERVICE_ROLE_KEY|OPENAI_API_KEY|ANTHROPIC_API_KEY)\b/,
  },
  {
    ...
    weight: 95,
    forceBlock: true,
    regex: /(?:ignore previous instructions|plain text instead of json|not valid json|no json object)/i,
  },
];
...
const normalizeAiRuntimeSecurityInputField = (value: unknown): AiRuntimeSecurityInputField | null => {
...
const capRiskScore = (value: number): number => Math.max(0, Math.min(100, Math.round(value)));
...
const redactSensitiveText = (value: string): string => (
  value
    ...
    .replace(/\b(?:SUPABASE_SERVICE_ROLE_KEY|OPENAI_API_KEY|ANTHROPIC_API_KEY)\b/g, "[redacted-secret-name]")
);

const clipText = (value: string, max = 220): string => value.length > max ? `${value.slice(0, max - 1)}…` : value;

const normalizeCityName = (value: string): string => normalizeWhitespace(value).toLocaleLowerCase();

const extractOutputCityNames = (parsedData: Record<string, unknown> | null | undefined): string[] => {
...
const routeOrderSatisfied = (outputCities: string[], destinationOrder: string[]): boolean => {
  ...
  for (const requiredCity of destinationOrder) {
    ...
    const foundIndex = outputCities.findIndex((city, index) => index >= currentIndex && city.includes(requiredName));
...
const buildPromptFingerprintSource = (input: AiRuntimeSecurityInput | null | undefined): string => {
  if (!input) return "";
  const parts = [
    input.flow || "",
...
const evaluateRules = (
  text: string,
  rules: DetectionRule[],
): { categories: Set<AiRuntimeSecurityAttackCategory>; matchedRules: Set<string>; riskScore: number; forceBlock: boolean; excerpt: string | null } => {
...
const buildSignal = async (params: {
  ...
  const blocked = params.forceBlock
    || attackCategories.some((category) => HIGH_CONFIDENCE_BLOCK_CATEGORIES.has(category))
    || params.riskScore >= 85;
  const guardDecision: AiRuntimeSecurityGuardDecision = blocked ? "block" : suspicious ? "warn" : "allow";
  ...
  return {
    ...
    attackCategories,
    matchedRules,
    promptFingerprintSha256: await sha256Hex(buildPromptFingerprintSource(params.input || null)),
    redactedExcerpt: params.excerpt || null,
  };
};

export const normalizeAiRuntimeSecurityInput = (value: unknown): AiRuntimeSecurityInput | null => {
  if (!isRecord(value)) return null;
...
export const buildAiRuntimeSecurityInputFromTripInputSnapshot = (value: unknown): AiRuntimeSecurityInput | null => {
  if (!isRecord(value)) return null;
  ...
  if (flow === "classic") {
    ...
    if (options) {
      ...
      if (specificCities) fields.push({ name: "specific_cities", value: specificCities });
...
export const detectAiRuntimeInputSecurity = async (
  ...
  normalizedInput.userFields.forEach((field) => {
    ...
    result.matchedRules.forEach((rule) => matchedRules.add(`${field.name}:${rule}`));
    riskScore += result.riskScore;
    if (result.forceBlock) forceBlock = true;
    if (!excerpt && result.excerpt) excerpt = `${field.name}: ${result.excerpt}`;
  });

  if (normalizedInput.routeLock && normalizedInput.destinationOrder.length > 0) {
...
export const detectAiRuntimeOutputSecurity = async (
  ...
  const rawText = typeof options.rawOutputText === "string" ? options.rawOutputText : "";
  ...
  if (options.forceSchemaBypass || (validation && (!validation.schemaValid || validation.errors.length > 0))) {
  ...
  if (parsedData && normalizedInput) {
    ...
    const missingRequiredCities = normalizedInput.requiredCities
      .map(normalizeCityName)
      .filter((city) => !outputCities.some((outputCity) => outputCity.includes(city)));
    ...
    if (normalizedInput.routeLock && normalizedInput.destinationOrder.length > 1 && !routeOrderSatisfied(outputCities, normalizedInput.destinationOrder)) {
      categories.add("constraint_override");
...
export const summarizeAiRuntimeSecuritySignals = (
  ...
  const filtered = signals.filter((signal): signal is AiRuntimeSecuritySignal => Boolean(signal));
  const primary = filtered
    ...
    .sort((left, right) => {
      ...
      return decisionScore(right.guardDecision) - decisionScore(left.guardDecision) || right.riskScore - left.riskScore;
    })[0] || {
  ...
  const attackCategories = [...new Set(filtered.flatMap((signal) => signal.attackCategories))];
  const matchedRules = [...new Set(filtered.flatMap((signal) => signal.matchedRules))];
  ...
  return {
    ...
    attackCategories,
    matchedRules,
    riskScore: capRiskScore(Math.max(...filtered.map((signal) => signal.riskScore), 0)),
    promptFingerprintSha256: filtered.find((signal) => signal.promptFingerprintSha256)?.promptFingerprintSha256 || null,
    redactedExcerpt: filtered.find((signal) => signal.redactedExcerpt)?.redactedExcerpt || null,
...
export const extractAiRuntimeSecurityMetadata = (value: unknown): AiRuntimeSecurityMetadata | null => {
  if (!isRecord(value)) return null;
  const attackCategories = Array.isArray(value.attackCategories)
    ? value.attackCategories.filter((entry): entry is AiRuntimeSecurityAttackCategory => (
      typeof entry === "string" && AI_RUNTIME_SECURITY_ATTACK_CATEGORIES.includes(entry as AiRuntimeSecurityAttackCategory)
    ))
    : [];
  const matchedRules = Array.isArray(value.matchedRules)
    ? value.matchedRules.filter((entry): entry is string => typeof entry === "string" && entry.trim().length > 0)
    : [];
  const stage = typeof value.stage === "string" && AI_RUNTIME_SECURITY_STAGES.includes(value.stage as AiRuntimeSecurityStage)
    ? value.stage as AiRuntimeSecurityStage
    : null;
  const guardDecision = typeof value.guardDecision === "string"
    && AI_RUNTIME_SECURITY_GUARD_DECISIONS.includes(value.guardDecision as AiRuntimeSecurityGuardDecision)
  ...
  return {
    ...
    suspicious: value.suspicious === true || attackCategories.length > 0 || guardDecision !== "allow",
    attackCategories,
    matchedRules,
    ...
    signals: Array.isArray(value.signals)
      ? value.signals
        ...
        .filter((signal): signal is AiRuntimeSecuritySignal => Boolean(signal))
      : undefined,
  };
};
