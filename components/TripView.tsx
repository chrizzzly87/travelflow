...
import {
  ...
  getTimelineBounds,
  getTripDistanceKm,
  isInternalMapColorModeControlEnabled,
  normalizeMapColorMode,
} from "../utils";
...
import {
  getExampleMapViewTransitionName,
  getExampleTitleViewTransitionName,
} from "../shared/viewTransitionNames";
...
import {
  buildVisualHistoryLabel,
  resolveVisualDiff,
  type ZoomChangeSource,
} from "./tripview/viewChangeDiff";
import {
  readFloatingMapPreviewState,
  writeFloatingMapPreviewState,
} from "./tripview/floatingMapPreviewState";
import {
...
const lazyWithRecovery = <
  TModule extends { default: React.ComponentType<any> },
>(
  moduleKey: string,
...
const ReleaseNoticeDialog = lazyWithRecovery("ReleaseNoticeDialog", () =>
  import("./ReleaseNoticeDialog").then((module) => ({
    default: module.ReleaseNoticeDialog,
  })),
);
...
const SelectedCitiesPanel = lazyWithRecovery("SelectedCitiesPanel", () =>
  import("./SelectedCitiesPanel").then((module) => ({
    default: module.SelectedCitiesPanel,
  })),
);

const TripDetailsDrawer = lazyWithRecovery("TripDetailsDrawer", () =>
  import("./TripDetailsDrawer").then((module) => ({
    default: module.TripDetailsDrawer,
  })),
);

const AddActivityModal = lazyWithRecovery("AddActivityModal", () =>
  import("./AddActivityModal").then((module) => ({
    default: module.AddActivityModal,
  })),
);
...
const TripShareModal = lazyWithRecovery("TripShareModal", () =>
  import("./TripShareModal").then((module) => ({
    default: module.TripShareModal,
  })),
);

const TripHistoryModal = lazyWithRecovery("TripHistoryModal", () =>
  import("./TripHistoryModal").then((module) => ({
    default: module.TripHistoryModal,
  })),
);

let tripInfoModalModulePromise: Promise<{
  default: React.ComponentType<any>;
}> | null = null;

const loadTripInfoModalModule = (): Promise<{
  default: React.ComponentType<any>;
}> => {
  if (!tripInfoModalModulePromise) {
    tripInfoModalModulePromise = import("./TripInfoModal").then((module) => ({
      default: module.TripInfoModal,
    }));
  }
...
const TRIP_CALENDAR_EXPORT_EVENT_BY_SCOPE: Record<
  ...
  | "trip_view__calendar_export--activity"
  | "trip_view__calendar_export--activities"
  | "trip_view__calendar_export--cities"
  | "trip_view__calendar_export--all"
> = {
...
const buildTripMetaSummary = (trip: ITrip): TripMetaSummary => {
  ...
  const maxEnd = cityItems.reduce(
    (max, city) => Math.max(max, city.startDateOffset + city.duration),
    0,
  );
  ...
  const distanceLabel =
    totalDistanceKm > 0
      ? formatDistance(totalDistanceKm, DEFAULT_DISTANCE_UNIT, {
...
export const extractTripTravelerWarnings = (
  items: ITimelineItem[],
): TripTravelerWarningSummary[] =>
  items
    .filter((item) => item.type === "city")
    .map((item) => {
      const description =
    ...
    .filter((warning): warning is TripTravelerWarningSummary =>
      Boolean(warning),
    );
...
const MapDeferredFallback: React.FC<{ onLoadNow: () => void }> = ({
  onLoadNow,
}) => (
...
const TripInfoModalLoadingFallback: React.FC<{ onClose: () => void }> = ({
  onClose,
}) => {
  ...
  return (
    <>
      ...
      >
        <div
          ref={dialogRef}
          className="pointer-events-auto bg-white rounded-t-2xl rounded-b-none sm:rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col max-h-[84vh] sm:max-h-[88vh]"
        >
          <div className="p-4 border-b border-gray-100 flex items-center justify-between">
            <div>
              <h3
                id="trip-info-loading-title"
                className="text-lg font-bold text-gray-900"
              >
                Trip information
              </h3>
              <p className="text-xs text-gray-500">
                Plan details, destination info, and history.
              </p>
            </div>
            <button
              ref={closeButtonRef}
            ...
            <div className="rounded-xl border border-gray-200 p-3 space-y-3 animate-pulse">
              ...
              <div className="grid grid-cols-2 gap-2">
                {TRIP_INFO_FALLBACK_ROWS.map((row) => (
                  <div
                    key={`trip-info-fallback-${row}`}
                    className="h-14 rounded-lg bg-gray-100 border border-gray-100"
                  />
                ))}
              </div>
            </div>
...
interface TripViewModalLayerProps {
  ...
  onPaywallActivateClick: (
    event: React.MouseEvent<HTMLAnchorElement>,
    analyticsEvent:
      | "trip_paywall__strip--activate"
      | "trip_paywall__overlay--activate",
...
const TripViewModalLayer: React.FC<TripViewModalLayerProps> = ({
  ...
  <>
    ...
    {isTripInfoOpen && (
      <Suspense
        fallback={<TripInfoModalLoadingFallback onClose={onCloseTripInfo} />}
      >
        <TripInfoModal
          ...
          onRetryGeneration={(modelId) =>
            onRetryGeneration("trip_info", modelId)
          }
...
interface RenderDetailsPanelContentOptions {
  ...
  onUpdateItem: (
    id: string,
    updates: Partial<ITimelineItem>,
    options?: { skipPendingLabel?: boolean },
  ) => void;
  onBatchUpdateItem: (...args: any[]) => void;
  onDeleteItem: (
    id: string,
    strategy?: "item-only" | "shift-gap" | "pull-back",
  ) => void;
...
const useTripViewRender = ({
  ...
  const { access, profile, isAuthenticated, isAnonymous, isAdmin, logout } =
    useAuth();
  ...
  const useExampleSharedTransition =
    trip.isExample && (locationState?.useExampleSharedTransition ?? true);
  const mapViewTransitionName = getExampleMapViewTransitionName(
    useExampleSharedTransition,
  );
  const titleViewTransitionName = getExampleTitleViewTransitionName(
    useExampleSharedTransition,
  );
  const tripRef = useRef(trip);
  const onUpdateTripRef = useRef(onUpdateTrip);
  const retryGenerationTabFeedbackSessionRef =
    useRef<TripGenerationTabFeedbackSession | null>(null);
  ...
  const [retryModelId, setRetryModelId] = useState<string>(
    getDefaultCreateTripModel().id,
  );
  ...
  const { adminOverrideEnabled, setAdminOverrideEnabled } =
    useTripAdminOverrideState();
  ...
  const effectiveReadOnly =
    readOnly || (isAdminFallbackView && !adminOverrideEnabled);
  const canEdit =
    !effectiveReadOnly && !isTripLockedByExpiry && !isTripLockedByArchive;
  ...
  const expectedCityLaneCount = displayTrip.items.filter(
    (item) => item.type === "city",
  ).length;
  const canEnableAdminOverride =
    isAdminFallbackView && Boolean(adminAccess?.canAdminWrite);
  const canUseAdminGenerationOverride =
    isAdminFallbackView &&
    adminOverrideEnabled &&
    Boolean(adminAccess?.canAdminWrite);
  ...
  const latestGenerationAttemptSecurity = useMemo(
    () =>
      extractAiRuntimeSecurityMetadata(
        latestGenerationAttemptMetadata?.security,
      ),
    [latestGenerationAttemptMetadata],
  );
  const isInputSecurityBlocked = useMemo(
    () =>
      isTripGenerationInputSecurityBlock(
        latestGenerationAttemptSecurity,
        latestGenerationAttempt?.errorCode || null,
      ),
    [latestGenerationAttempt?.errorCode, latestGenerationAttemptSecurity],
  );
  const isOutputSecurityBlocked = useMemo(
    () =>
      isTripGenerationOutputSecurityBlock(
        latestGenerationAttemptSecurity,
        latestGenerationAttempt?.errorCode || null,
  ...
  const securityRecoveryFields = useMemo(
    () =>
      getTripGenerationSecurityRecoveryFields(
        generationInputSnapshot,
        latestGenerationAttemptSecurity,
      ),
    [generationInputSnapshot, latestGenerationAttemptSecurity],
  );
  const securityRecoveryFieldLabels = useMemo<
    Record<TripGenerationSecurityRecoveryFieldKey, string>
  >(
    () => ({
      trip_request: t(
        "tripView.generation.securityRecovery.fields.trip_request",
      ),
      destination: t("tripView.generation.securityRecovery.fields.destination"),
      destinations: t(
        "tripView.generation.securityRecovery.fields.destinations",
      ),
      specific_cities: t(
        "tripView.generation.securityRecovery.fields.specific_cities",
      ),
      notes: t("tripView.generation.securityRecovery.fields.notes"),
      seasonal_events: t(
        "tripView.generation.securityRecovery.fields.seasonal_events",
      ),
    }),
    [t],
  );
  const [securityRecoveryDraft, setSecurityRecoveryDraft] = useState<
    Partial<Record<TripGenerationSecurityRecoveryFieldKey, string>>
  >({});
  const [isSecurityRecoveryReviewOpen, setIsSecurityRecoveryReviewOpen] =
    useState(false);
  const securityRecoveryFieldViews = useMemo(
    () =>
      securityRecoveryFields.map((field) => ({
        ...field,
        label: securityRecoveryFieldLabels[field.key] || field.key,
      })),
    [securityRecoveryFieldLabels, securityRecoveryFields],
  );
  const hasSecurityRecoveryChanges = useMemo(
    () =>
      hasTripGenerationSecurityRecoveryChanges(
        securityRecoveryFields,
        securityRecoveryDraft,
      ),
    [securityRecoveryDraft, securityRecoveryFields],
  );
  const securityRecoveryUpdatedSnapshot = useMemo(() => {
    if (!generationInputSnapshot) return null;
    return applyTripGenerationSecurityRecoveryUpdates(
      generationInputSnapshot,
      securityRecoveryDraft,
    );
  }, [generationInputSnapshot, securityRecoveryDraft]);
  const canRetryWithSecurityRecovery = useMemo(
    () =>
      hasSecurityRecoveryChanges &&
      isTripGenerationSecurityRecoverySnapshotValid(
        securityRecoveryUpdatedSnapshot,
      ),
    [hasSecurityRecoveryChanges, securityRecoveryUpdatedSnapshot],
  );
  ...
  const pendingAuthQueueRequestId = useMemo(() => {
    ...
    const queueRequestId =
      typeof queueRequestValue === "string" ? queueRequestValue.trim() : "";
    if (!queueRequestId) return null;
    const pendingAuthValue = metadata.pendingAuth;
    return pendingAuthValue === true ? queueRequestId : null;
  }, [latestGenerationAttempt?.metadata]);
  const isPendingAuthGeneration =
    generationState === "failed" && Boolean(pendingAuthQueueRequestId);
  useEffect(() => {
    setSecurityRecoveryDraft({});
    setIsSecurityRecoveryReviewOpen(
      isInputSecurityBlocked && securityRecoveryFields.length > 0,
    );
  }, [
    isInputSecurityBlocked,
    latestGenerationAttempt?.id,
    securityRecoveryFields.length,
  ]);
  ...
  const shouldPollGenerationState = shouldPollTripGenerationState(
    trip,
    generationNowMs,
  );
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      kickWorker,
      TRIP_GENERATION_QUEUE_NUDGE_INTERVAL_MS,
    );
    return () => {
      cancelled = true;
      window.clearInterval(timer);
  ...
  const [
    ...
    setIsResolvingPendingAuthGeneration,
  ] = useState(false);
  const [hasSeenPendingAuthLoadingState, setHasSeenPendingAuthLoadingState] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (
      ...
      generationElapsedMs < TRIP_GENERATION_TIMEOUT_MS + 20_000
    ) {
      return;
    }
    ...
    const recoverStalledAttempt = async () => {
      try {
        ...
        if (stallRecoveryAction === "nudge_worker") {
          const lastNudgeWasForSameAttempt =
            asyncStallRecoveryNudgeAttemptIdRef.current ===
            latestGenerationAttempt.id;
          const nudgeCooldownMs = 15_000;
          if (
            lastNudgeWasForSameAttempt &&
            nowMs - asyncStallRecoveryNudgeAtRef.current < nudgeCooldownMs
          ) {
            return;
          }
          asyncStallRecoveryNudgeAttemptIdRef.current =
        ...
        const failedTrip = markTripGenerationFailed(tripRef.current, {
          ...
          provider:
            ...
            tripRef.current.aiMeta?.provider ||
            null,
          model:
            latestGenerationAttempt.model ||
            tripRef.current.aiMeta?.model ||
            null,
          ...
          error: {
            ...
            status: 504,
            message:
              "Generation queue job was not active anymore. Please retry.",
            failureKind: "timeout",
          },
          metadata: {
        ...
        await finishTripGenerationAttemptLog({
          ...
          provider:
            latestGenerationAttempt.provider ||
            failedTrip.aiMeta?.provider ||
            null,
          model:
            latestGenerationAttempt.model || failedTrip.aiMeta?.model || null,
          providerModel: latestGenerationAttempt.providerModel || null,
          requestId: latestGenerationAttempt.requestId || null,
          durationMs:
            failedTrip.aiMeta?.generation?.latestAttempt?.durationMs ||
          ...
          errorMessage:
            "Generation queue job was not active anymore. Please retry.",
          finishedAt:
            failedTrip.aiMeta?.generation?.latestAttempt?.finishedAt ||
            undefined,
          metadata: {
            source: "trip_view_async_stall_recovery",
          },
  ...
  useEffect(() => {
    ...
    if (isGenerationInFlight) {
      ...
      if (!session && tripAccess?.source !== "public_read") {
        retryGenerationTabFeedbackSessionRef.current =
          beginTripGenerationTabFeedback();
      }
      return;
    }
    ...
    if (!session) return;
    if (
      pendingRetryGenerationStateRef.current &&
  ...
  const isPendingAuthClaimedByAnotherUser =
    pendingAuthClaimConflict === TRIP_CLAIM_CONFLICT_ALREADY_CLAIMED;
  ...
  const pendingAuthModalStage: "hidden" | "loading" | "locked" =
    !isPendingAuthGeneration || isPendingAuthClaimedByAnotherUser
      ? "hidden"
      : isResolvingPendingAuthGeneration || !hasSeenPendingAuthLoadingState
        ? "loading"
        : "locked";
  const hasSuccessfulGeneration =
  ...
  const handleResolvePendingAuthGeneration = useCallback(async () => {
    ...
    if (isAuthenticated && !isAnonymous) {
      ...
      try {
        const result = await processQueuedTripGenerationAfterAuth(
          pendingAuthQueueRequestId,
        );
        registerTripGenerationCompletionWatch(
          result.tripId,
          "auth_queue_claim_trip_view",
        );
        if (!isMobileViewport && !suppressToasts) {
          showAppToast({
            tone: "add",
            ...
            description:
              "Trip generation started and is running in the background.",
          });
        }
          ...
          showAppToast({
            ...
            description:
              error instanceof Error
                ? error.message
                : "Could not start trip generation.",
          });
        }
        autoClaimedPendingAuthQueueRequestIdRef.current = null;
      } finally {
  ...
  useEffect(() => {
    ...
    if (
      autoClaimedPendingAuthQueueRequestIdRef.current ===
      pendingAuthQueueRequestId
    )
      return;
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(
      new CustomEvent<ViewTransitionDebugDetail>(VIEW_TRANSITION_DEBUG_EVENT, {
        detail: {
          phase: "trip-view-mounted",
          templateId: trip.exampleTemplateId,
          useExampleSharedTransition,
          expectedCityLaneCount,
          reason: mapViewTransitionName
            ? undefined
            : "shared-map-anchor-disabled",
        },
      }),
    );
  }, [
  ...
  const cityColorPaletteId =
    trip.cityColorPaletteId || DEFAULT_CITY_COLOR_PALETTE_ID;
  ...
  const navigateHistoryRef = useRef<
    | ((action: "undo" | "redo", options?: { silent?: boolean }) => boolean)
    | null
  >(null);
  ...
  const [mapDockMode, setMapDockMode] = useState<"docked" | "floating">(() => {
    if (
      initialViewSettings?.mapDockMode === "floating" ||
      initialViewSettings?.mapDockMode === "docked"
    ) {
      return initialViewSettings.mapDockMode;
    }
    const persisted = readFloatingMapPreviewState().mode;
    return persisted === "floating" || persisted === "docked"
      ? persisted
      : "docked";
  });
  ...
  const markZoomDirty = useCallback(
    (source: ZoomChangeSource = "manual") => {
      if (source) {
  ...
  useEffect(() => {
    const handleSyncToast = (event: Event) => {
      const detail = (event as CustomEvent<SyncToastEventDetail | undefined>)
        .detail;
      if (!detail) return;
    ...
    window.addEventListener(
      TRIP_SYNC_TOAST_EVENT,
      handleSyncToast as EventListener,
    );
    return () => {
  ...
  const canAbortAndRetryGeneration =
    isGenerationSlow &&
    canTriggerTripGenerationAbortAndRetry({
      canEdit,
      isAdminFallbackView,
  ...
  const retryDisabledReason = useMemo(() => {
    ...
    if (isInputSecurityBlocked) {
      return t("tripView.generation.securityRecovery.changeHint");
    }
  ...
  const resolvedGenerationFailureMessage = useMemo(() => {
    if (isInputSecurityBlocked) {
      return t("tripView.generation.securityRecovery.blockedMessage");
    }
    if (isOutputSecurityBlocked) {
      return t("tripView.generation.securityRecovery.outputBlockedMessage");
    }
    return latestGenerationAttempt?.errorMessage || null;
  }, [
    isInputSecurityBlocked,
    isOutputSecurityBlocked,
  ...
  const handleSecurityRecoveryFieldChange = useCallback(
    (key: TripGenerationSecurityRecoveryFieldKey, value: string) => {
      setSecurityRecoveryDraft((current) => {
  ...
  const handleOpenSecurityRecoveryReview = useCallback(() => {
    setIsSecurityRecoveryReviewOpen(true);
  }, []);

  const handleCancelSecurityRecoveryReview = useCallback(() => {
    setIsSecurityRecoveryReviewOpen(false);
  }, []);

  const handleClearSecurityRecoveryFields = useCallback(() => {
    if (!generationInputSnapshot || securityRecoveryFields.length === 0) return;
    const clearedSnapshot = clearTripGenerationSecurityRecoveryFields(
      generationInputSnapshot,
      securityRecoveryFields.map((field) => field.key),
    );
    const nextDraft = securityRecoveryFields.reduce<
      Partial<Record<TripGenerationSecurityRecoveryFieldKey, string>>
    >((accumulator, field) => {
      const originalValue =
        field.key in securityRecoveryDraft
          ? securityRecoveryDraft[field.key] || ""
          : field.value;
      if (!originalValue.trim()) return accumulator;
      accumulator[field.key] = "";
      return accumulator;
    }, {});

    setSecurityRecoveryDraft(nextDraft);
    setIsSecurityRecoveryReviewOpen(true);

    if (!isTripGenerationSecurityRecoverySnapshotValid(clearedSnapshot)) {
      showToast(t("tripView.generation.securityRecovery.requiredFieldHint"), {
        tone: "neutral",
        title: t("tripView.generation.securityRecovery.clearAction"),
      });
    }
  }, [
    generationInputSnapshot,
  ...
  const handleRetryGeneration = useCallback(
    async (
      ...
      if (!(canEdit || canUseAdminGenerationOverride)) {
        const message =
          isAdminFallbackView && !adminOverrideEnabled
      ...
      try {
        const result = await retryTripGenerationWithDefaultModel(baseTrip, {
          source:
            source === "trip_info" ? "trip_info_modal" : "trip_status_strip",
        ...
        showToast(
          error instanceof Error
            ? error.message
            : t("tripView.generation.retry.unexpected"),
          {
            tone: "neutral",
            title: t("tripView.generation.retry.failedTitle"),
          },
  ...
  const handleRetryWithSecurityRecovery = useCallback(async () => {
    if (!generationInputSnapshot || !securityRecoveryUpdatedSnapshot) return;
    if (
      !hasSecurityRecoveryChanges ||
      !isTripGenerationSecurityRecoverySnapshotValid(
        securityRecoveryUpdatedSnapshot,
      ...
      showToast(t("tripView.generation.securityRecovery.changeHint"), {
        tone: "neutral",
        title: t("tripView.generation.securityRecovery.cardTitle"),
      });
      return;
    }
    ...
    const updatedTrip: ITrip = {
      ...
      aiMeta: {
        ...
        generation: {
          ...
          inputSnapshot: securityRecoveryUpdatedSnapshot,
        },
      },
    };

    await handleRetryGeneration("trip_strip", retryModelId, updatedTrip);
  }, [
    generationInputSnapshot,
  ...
  const handleAbortAndRetryGeneration = useCallback(async () => {
    ...
    const abortedTrip = markTripGenerationFailed(trip, {
      ...
      provider:
        latestGenerationAttempt?.provider || trip.aiMeta?.provider || null,
  ...
  const requireEdit = useCallback(() => {
    if (isTripLockedByArchive) {
      showToast(
        "Archived trips stay read-only. Unarchive in Admin Trips first.",
        { tone: "neutral", title: "Archived trip" },
      );
      return false;
    }
    if (isTripLockedByExpiry) {
      if (isAdminFallbackView) {
        showToast("Expired trips stay read-only in admin override mode.", {
          tone: "neutral",
          title: "Expired trip",
        });
        return false;
      }
      showToast("Trip expired. Activate with an account to edit again.", {
        tone: "neutral",
        title: "Expired trip",
      });
      return false;
    ...
    if (isAdminFallbackView && !adminOverrideEnabled) {
      showToast("Admin read-only mode is active. Enable override to edit.", {
        tone: "neutral",
        title: "Read only",
      });
      return false;
    }
    showToast("View-only link. Copy the trip to edit.", {
      tone: "neutral",
      title: "View only",
    });
    return false;
  ...
  const showSavedToastForLabel = useCallback(
    (label: string) => {
  ...
  const {
    ...
    tripId: trip.id,
    tripUpdatedAt:
      typeof trip.updatedAt === "number" ? trip.updatedAt : undefined,
    locationPathname: location.pathname,
    currentUrl,
    isExamplePreview,
    navigate,
  ...
  const resolveMapDockModeLabel = useCallback(
    (fromMode: "docked" | "floating", toMode: "docked" | "floating"): string =>
  ...
  useEffect(() => {
    const cityIdSet = new Set(
      trip.items.filter((item) => item.type === "city").map((item) => item.id),
    );
    ...
    if (
      selectedItemId &&
      !trip.items.some((item) => item.id === selectedItemId)
    ) {
  ...
  const loadingDestinationSummary = useMemo(() => {
    const locations = displayTrip.items
      .filter(
        (item) => item.type === "city" && typeof item.location === "string",
      )
  ...
  const effectiveMapFocusQuery = useMemo(() => {
    const queries = buildTripMapLocationContextQueries(
      trip,
      initialMapFocusQuery,
    );
    return queries.join(" || ");
  ...
  const shouldEnableReleaseNotice =
    isReleaseNoticeReady &&
  ...
  const forkMeta = useMemo(() => {
    if (trip.forkedFromShareToken) {
      return {
        ...
        url: buildShareUrl(
          trip.forkedFromShareToken,
          trip.forkedFromShareVersionId ?? null,
        ),
      };
    }
    ...
    trip.forkedFromShareToken,
    trip.forkedFromShareVersionId,
    trip.forkedFromTripId,
  ]);
  ...
  const { historyModalItems } = useTripHistoryPresentation({
    currentUrl,
    displayHistoryEntries,
    refreshHistory,
    setIsHistoryOpen,
  ...
  const scheduleCommit = useCallback(
    (
      ...
      debugHistory("Scheduled commit", {
        label: pendingHistoryLabelRef.current || "Data: Updated trip",
      });

      if (commitTimerRef.current) clearTimeout(commitTimerRef.current);
      const pendingLabel =
        pendingHistoryLabelRef.current || "Data: Updated trip";
  ...
  useEffect(() => {
    ...
    const { changes, didZoomChange, isAutoZoomOnlyChange } = resolveVisualDiff({
      previous: prev,
    ...
    const hasOnlyZoomLabel =
      changes.length === 1 &&
      (changes[0] === "Zoomed in" || changes[0] === "Zoomed out");
    if (
      didZoomChange &&
      hasOnlyZoomLabel &&
      zoomChangeSourceRef.current !== "manual" &&
      !isZoomDirty
    ) {
      prevViewRef.current = currentViewSettings;
      zoomChangeSourceRef.current = null;
    ...
    const nextVisualLabel = buildVisualHistoryLabel(
      pendingHistoryLabelRef.current,
      changes,
    );
    if (nextVisualLabel) {
  ...
  const effectiveLayoutMode: "vertical" | "horizontal" = isMobile
    ? "vertical"
    : layoutMode;
  const effectiveMapDockMode: "docked" | "floating" = isMobile
    ? "docked"
    : mapDockMode;
  ...
  const { handleForceFill, selectedCityForceFill } = useTripCityForceFill({
    tripItems: trip.items,
    selectedItemId,
    setPendingLabel,
    handleUpdateItems,
  ...
  const {
    ...
    onUndoDelete: (deletedItem, context) => {
      const deletedEntityLabel =
        deletedItem.type === "city"
  ...
  useLayoutEffect(() => {
    ...
    const rebalanceDockedPaneWidths = () => {
      const availableWidth =
        window.innerWidth -
        (detailsPanelVisible ? RESIZER_WIDTH * 2 : RESIZER_WIDTH);
      const targetPaneWidth = Math.floor(
        availableWidth / (detailsPanelVisible ? 3 : 2),
      );
  ...
  const focusCityBlockById = useCallback((cityId: string) => {
    ...
    requestAnimationFrame(() => {
      const nextBlock = Array.from(
        document.querySelectorAll<HTMLElement>("[data-city-id]"),
  ...
  const handleNavigateSelectedCity = useCallback(
    (direction: "previous" | "next") => {
      const activeCityId =
        selectedItemId && orderedCityIds.includes(selectedItemId)
          ? selectedItemId
  ...
  const resolveSteppedZoomLevel = useCallback(
    (currentZoomLevel: number, direction: "in" | "out") => {
      const normalizedPresets = TIMELINE_ZOOM_LEVEL_PRESETS.map((value) =>
        clampZoomLevel(value),
      )
      ...
      if (direction === "in") {
        return (
          normalizedPresets.find(
            (preset) => preset > currentZoomLevel + 0.001,
          ) ??
  ...
  const { mapViewportRef, isMapBootstrapEnabled, enableMapBootstrap } =
  ...
  const showOwnedTripConnectivityStatus =
    !shareStatus && !isExamplePreview && !isAdminFallbackView;
  const latestConflictBackupEntry = useMemo(() => {
    ...
    showOwnedTripConnectivityStatus,
    syncSnapshot.hasConflictBackups,
    syncSnapshot.lastRunAt,
    trip.id,
  ]);
  ...
  const ownerSummary = useMemo(() => {
    ...
    if (ownerUsername) {
      const normalizedHandle = ownerUsername.startsWith("@")
        ? ownerUsername
        : `@${ownerUsername}`;
    ...
    if (isOwner && currentUsername) {
      const normalizedHandle = currentUsername.startsWith("@")
        ? currentUsername
        : `@${currentUsername}`;
  ...
  const tripOwnerAdminMeta = useMemo(() => {
    ...
    const ownerUserId =
      tripAccess?.ownerId?.trim() || access?.userId?.trim() || null;
    const ownerUsername =
      tripAccess?.ownerUsername?.trim() || profile?.username?.trim() || null;
    const ownerEmail =
      tripAccess?.ownerEmail?.trim() || access?.email?.trim() || null;
  ...
  const handleTimelineTaskToggle = useCallback(
    (itemId: string, taskLineNumber: number, checked: boolean) => {
      const item = trip.items.find((candidate) => candidate.id === itemId);
      if (!item || typeof item.description !== "string") return;
      const nextDescription = toggleMarkdownTaskByLine(
        item.description,
        taskLineNumber,
        checked,
      );
  ...
  const { handleStartTitleEdit, handleCommitTitleEdit, handleCancelTitleEdit } =
    useTripTitleEditHandlers({
      canManageTripMetadata,
      isMobile,
      isEditingTitle,
      editTitleValue,
  ...
  const handleExportSelectedActivityCalendar = useCallback(
    (itemId: string) => {
  ...
  const handleExportActivitiesCalendar = useCallback(
    (source: "trip_info_modal" | "print_view") => {
      handleTripCalendarExport("activities", source);
    },
    [handleTripCalendarExport],
  );

  const handleExportCitiesCalendar = useCallback(
    (source: "trip_info_modal" | "print_view") => {
  ...
  const handleExportAllCalendar = useCallback(
    (source: "trip_info_modal" | "print_view") => {
  ...
  const detailsPanelContent = renderDetailsPanelContent({
    showSelectedCitiesPanel,
    selectedCitiesInTimeline,
    onCloseSelection: showSelectedCitiesPanel
      ? clearSelection
      : closeDetailsPanel,
    ...
    cityColorPaletteId,
    onCityColorPaletteChange: canEdit
      ? handleCityColorPaletteChange
      : undefined,
  ...
  if (viewMode === "print") {
    return (
      <GoogleMapsLoader language={appLanguage}>
        <Suspense
          fallback={
            <div className="h-screen w-screen flex items-center justify-center text-sm text-gray-500">
        ...
        >
          <PrintLayout
            ...
            onExportActivitiesCalendar={() =>
              handleExportActivitiesCalendar("print_view")
            }
            onExportCitiesCalendar={() =>
              handleExportCitiesCalendar("print_view")
            }
    ...
    <GoogleMapsLoader language={appLanguage} enabled={isMapBootstrapEnabled}>
      ...
      >
        ...
        <TripViewStatusBanners
          ...
          connectivityReason={
            showOwnedTripConnectivityStatus ? connectivitySnapshot.reason : null
          }
          connectivityForced={
            showOwnedTripConnectivityStatus
          ...
          pendingSyncCount={
            showOwnedTripConnectivityStatus ? syncSnapshot.pendingCount : 0
          }
          failedSyncCount={
            showOwnedTripConnectivityStatus ? syncSnapshot.failedCount : 0
          }
          isSyncingQueue={
            showOwnedTripConnectivityStatus ? syncSnapshot.isSyncing : false
          }
          onRetrySyncQueue={
            showOwnedTripConnectivityStatus
              ? () => {
          ...
          hasConflictBackupForTrip={
            showOwnedTripConnectivityStatus
              ? Boolean(latestConflictBackupEntry)
              : false
          }
          onRestoreConflictBackup={
            showOwnedTripConnectivityStatus && latestConflictBackupEntry
          ...
          generationFailureMessage={resolvedGenerationFailureMessage}
          ...
          securityRecovery={securityRecoveryBanner}
        ...
        <main className="flex-1 relative overflow-hidden flex flex-col">
          <TripViewPlannerWorkspace
            ...
            onToggleMobileMapExpanded={() =>
              setIsMobileMapExpanded((value) => !value)
            }
            ...
            onTimelineModeChange={(mode) => {
              ...
              trackEvent(
                mode === "calendar"
                  ? "trip_view__mode--calendar"
                  : "trip_view__mode--timeline",
                {
                  trip_id: trip.id,
                },
              );
            ...
            onMapColorModeChange={
              allowMapColorModeControls ? handleMapColorModeChange : undefined
            }
          ...
          <TripViewModalLayer
            ...
            onCloseAddActivity={() =>
              setAddActivityState({ ...addActivityState, isOpen: false })
            }
            onAddActivity={handleAddActivityItem}
            trip={trip}
            isAddCityModalOpen={isAddCityModalOpen}
            onCloseAddCityModal={() => setIsAddCityModalOpen(false)}
            onAddCity={(name, lat, lng) =>
              handleAddCityItem({ title: name, coordinates: { lat, lng } })
            }
            ...
            canOpenBenchmarkWithSnapshot={Boolean(
              displayTrip.aiMeta?.generation?.inputSnapshot,
            )}
            ...
            travelerWarnings={travelerWarnings}
            onExportActivitiesCalendar={() =>
              handleExportActivitiesCalendar("trip_info_modal")
            }
            onExportCitiesCalendar={() =>
              handleExportCitiesCalendar("trip_info_modal")
            }
            onExportAllCalendar={() =>
              handleExportAllCalendar("trip_info_modal")
            }
            onOpenPrintLayout={() => {
              closeTripInfoModal();
              setViewMode("print");
