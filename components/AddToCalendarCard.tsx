...
export const AddToCalendarCard: React.FC<AddToCalendarCardProps> = ({ config, postSlug }) => {
    ...
    return (
        <section className="my-10 rounded-2xl border border-border bg-card p-5 shadow-sm md:p-6 lg:-mx-8 lg:rounded-3xl lg:px-7 xl:-mx-10 dark:shadow-none">
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0 flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                        <p className="text-xs font-semibold uppercase tracking-wide text-accent-700 dark:text-accent-200">
                            {titleLabel}
                        </p>
                        <h3 className="mt-1 text-lg font-semibold text-foreground">{config.title}</h3>
                        {config.description ? <p className="mt-1 text-sm text-muted-foreground">{config.description}</p> : null}
                        <p className="mt-1 text-xs font-medium text-muted-foreground">{eventCountLabel}</p>
                        <p className="mt-0.5 text-xs font-medium text-muted-foreground">{compatibleHint}</p>
                    </div>
                </div>
                {icsHref ? (
                    <a
                    ...
                    <button
                        type="button"
                        disabled
                        className="inline-flex items-center gap-1.5 rounded-lg border border-accent-200 bg-accent-50 px-3 py-2 text-sm font-semibold text-accent-800 opacity-60 dark:bg-accent-400/12 dark:text-accent-200 dark:border-accent-400/30"
                    >
                        <DownloadSimple size={14} weight="bold" />
                        <span>{downloadLabel}</span>
                    </button>
