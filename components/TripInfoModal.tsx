...
export interface TripInfoModalProps {
  ...
  historyItems?: TripInfoHistoryItem[];
  onGoToHistoryEntry: (entry: TripInfoHistoryItem) => void;
...
export const TripInfoModal: React.FC<TripInfoModalProps> = ({
  ...
  const [uncontrolledHistoryExpanded, setUncontrolledHistoryExpanded] = useState(false);
  ...
  const selectedRetryModelOption =
    activeRetryModelOptions.find((option) => option.id === selectedRetryModelId) || null;
    ...
    (isRetryingGeneration
      ...
      : !aiMeta?.generation?.inputSnapshot
        ...
        : !canRetryGeneration
          ...
          : null);
  const resolvedInfoHistoryItems = infoHistoryItems || historyItems || [];
  const resolvedHistoryExpanded =
    typeof isTripInfoHistoryExpanded === 'boolean'
      ? isTripInfoHistoryExpanded
      : uncontrolledHistoryExpanded;
  const handleToggleHistoryExpanded = () => {
    if (onToggleTripInfoHistoryExpanded) {
      onToggleTripInfoHistoryExpanded();
      return;
    }
    setUncontrolledHistoryExpanded((value) => !value);
  };
  ...
  const hasFailedSync = failedSyncCount > 0;
    ...
    >
          ...
          {canManageTripMetadata && (
            <div className="shrink-0 flex items-center gap-2">
              ...
              >
                <Star
                  size={16}
                  className={isFavorite ? 'fill-amber-400 text-amber-500' : 'text-gray-300 hover:text-amber-500'}
                />
              </button>
            </div>
          )}
        ...
        <section className="rounded-xl border border-slate-200 bg-slate-50 p-3">
          ...
          <dl className="grid grid-cols-1 gap-2 text-xs sm:grid-cols-2">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-2">
              ...
              <dd className="mt-1 break-all font-mono text-[11px] font-semibold text-slate-900">
      ...
      <section className="rounded-xl border border-gray-200 p-3">
        ...
        <dl className="grid grid-cols-2 gap-2 text-xs">
          ...
          {generationPill && (
            <div className="col-span-2 rounded-lg border border-gray-100 bg-gray-50 p-2">
        ...
        {ownerHint && <p className="mt-2 text-[11px] text-gray-600">{ownerHint}</p>}
      </section>
      ...
      {travelerWarnings.length > 0 && (
        <section className="rounded-xl border border-amber-200 bg-amber-50/70 p-3">
          ...
          <p className="mt-1 text-xs leading-5 text-amber-800">{t('tripView.warningSummary.description')}</p>
      ...
      <section className="rounded-xl border border-gray-200 p-3">
        ...
        <div className={`grid grid-cols-1 gap-2 ${onOpenPrintLayout ? 'sm:grid-cols-4' : 'sm:grid-cols-3'}`}>
          <button
            type="button"
        ...
        <section className="rounded-xl border border-gray-200 p-3">
            ...
            <div className="mt-3 space-y-1.5 rounded-lg border border-gray-100 bg-gray-50 p-2">
              ...
              <ul className="space-y-1">
                {recentAttempts.map((attempt) => (
                  <li
                    key={attempt.id}
                    className="flex items-center justify-between gap-2 rounded-md border border-gray-100 bg-white px-2 py-1 text-[11px] text-gray-600"
                  >
          ...
          {onRetryGeneration && aiMeta?.generation && (
            <div className="mt-3">
              ...
              <span className="inline-flex" title={resolvedRetryDisabledReason || t('tripView.generation.tripInfo.retry')}>
                <button
                  type="button"
                  ...
                  {...(retryAnalyticsAttributes || {})}
                >
                  {isRetryingGeneration ? t('tripView.generation.tripInfo.retrying') : t('tripView.generation.tripInfo.retry')}
                </button>
              </span>
      ...
      <section className="rounded-xl border border-gray-200 p-3">
        <button
          type="button"
          onClick={handleToggleHistoryExpanded}
          className="flex w-full items-center justify-between text-sm font-semibold text-gray-800"
        >
          <span>History</span>
          {resolvedHistoryExpanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
        </button>
        {resolvedHistoryExpanded && (
            ...
            {isExamplePreview ? (
              ...
              <>
                {(hasPendingSync || hasFailedSync) && (
                  <div className="space-y-1 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
                    ...
                    {hasFailedSync && (
                      <p>
                        {failedSyncCount === 1
                          ? '1 recent change could not be synced yet.'
                          : `${failedSyncCount} recent changes could not be synced yet.`}
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="overflow-hidden rounded-lg border border-gray-100">
                  {resolvedInfoHistoryItems.length === 0 ? (
                    ...
                    <ul className="max-h-56 divide-y divide-gray-100 overflow-y-auto">
                      {resolvedInfoHistoryItems.map((entry) => (
                        <li
                          key={entry.id}
                          className={`flex items-start gap-2 p-3 ${entry.isCurrent ? 'bg-accent-50/70' : 'bg-white'}`}
                        >
                          <div className="min-w-0 flex-1">
                          ...
                          <button
                            type="button"
                            onClick={() => onGoToHistoryEntry(entry)}
