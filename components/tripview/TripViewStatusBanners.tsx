...
import type { TripGenerationSecurityRecoveryFieldKey } from "../../services/tripGenerationSecurityRecoveryService";
...
interface TripViewStatusBannersProps {
  ...
  securityRecovery?: {
    ...
    onClearFlaggedText?: () => void;
    onRetryWithChanges?: () => void;
  };
  exampleTripBanner?: {
    title: string;
    countries: string[];
...
export const TripViewStatusBanners: React.FC<TripViewStatusBannersProps> = ({
  ...
  const shouldShowConnectivityStrip = Boolean(
    connectivityState && connectivityState !== "online",
  );
  const shouldShowSyncStrip = pendingSyncCount > 0 || isSyncingQueue;
  const showSyncStatusStrip =
    shouldShowConnectivityStrip || shouldShowSyncStrip;
  ...
  const supportContactPath = buildLocalizedMarketingPath(
    "contact",
    activeLocale,
  );
  const explorerHighlights = React.useMemo(() => {
    ...
    const interpolationValues = {
      maxActiveTripsLabel:
        explorerTier.entitlements.maxActiveTrips === null
          ? unlimitedLabel
          : String(explorerTier.entitlements.maxActiveTrips),
      maxTotalTripsLabel:
        explorerTier.entitlements.maxTotalTrips === null
          ? unlimitedLabel
          : String(explorerTier.entitlements.maxTotalTrips),
      tripExpirationLabel:
        explorerTier.entitlements.tripExpirationDays === null
          ? noExpiryLabel
          : t("shared.days", {
              ns: "pricing",
              count: explorerTier.entitlements.tripExpirationDays,
            }),
      sharingLabel: explorerTier.entitlements.canShare
        ? enabledLabel
        : disabledLabel,
      editableSharesLabel: explorerTier.entitlements.canCreateEditableShares
        ? enabledLabel
        : disabledLabel,
      proCreationLabel: explorerTier.entitlements.canCreateProTrips
        ? enabledLabel
        : disabledLabel,
    };
    return [0, 2, 4].map((index) =>
      t(`tiers.explorer.features.${index}`, {
        ns: "pricing",
        ...interpolationValues,
      }),
    );
  ...
  const stripMessage = shouldShowConnectivityStrip
    ...
    : isSyncingQueue
      ? t(`connectivity.tripStrip.syncing${syncCountVariant}`, {
          count: pendingSyncCount,
        })
      : t(`connectivity.tripStrip.pending${syncCountVariant}`, {
          count: pendingSyncCount,
        });
  ...
  const isInputSecurityRecoveryActive =
    generationState === "failed" &&
    !isPendingAuthGeneration &&
    Boolean(securityRecovery);
  const shouldShowGenerationStrip =
    generationState === "running" ||
    generationState === "queued" ||
    (generationState === "failed" && !isPendingAuthGeneration);
  const generationStripMessage =
    generationState === "failed"
      ? isPendingAuthGeneration
        ? t("tripPaywall.strip.loginNoDate")
        : isInputSecurityRecoveryActive
          ? t("tripView.generation.securityRecovery.blockedMessage")
          : generationFailureMessage ||
            t("tripView.generation.strip.failedDefault")
      : isSlowGeneration
  ...
  return (
    <>
      {showSyncStatusStrip && (
        <div
          className={`px-4 py-2 text-xs sm:px-6 border-b flex items-center justify-between gap-3 ${
            connectivityState === "offline"
        ...
        >
          <span className="inline-flex items-center gap-2">
            {stripIcon}
            {stripMessage}
            {connectivityForced && IS_DEV
              ? ` ${t("connectivity.tripStrip.forcedSuffix")}`
              : ""}
          </span>
          <div className="flex items-center gap-2">
            {showContactAction && (
              <a
                ...
                {...getAnalyticsDebugAttributes(
                  "trip_connectivity__banner--contact",
                  {
                    trip_id: tripId,
                    pending_count: pendingSyncCount,
                    connectivity_state: connectivityState,
            ...
            {failedSyncCount > 0 && onRetrySyncQueue && (
              <button
                ...
                className="px-3 py-1 rounded-md bg-white text-xs font-semibold border border-current/20 hover:bg-white/80"
                {...getAnalyticsDebugAttributes(
                  "trip_connectivity__trip_strip--retry_sync",
                  {
                    trip_id: tripId,
                    failed_count: failedSyncCount,
                    pending_count: pendingSyncCount,
      ...
      {shouldShowGenerationStrip && (
        ...
        >
          <span className="inline-flex items-center gap-2">
            ...
            {generationStripMessage}
          </span>
          <div className="flex items-center gap-2">
            {isSlowGeneration &&
              canAbortAndRetryGeneration &&
              onAbortAndRetryGeneration && (
                <button
                  type="button"
                  onClick={() => {
                  ...
                  {...getAnalyticsDebugAttributes(
                    "trip_generation__trip_strip--abort_retry",
                    {
                      trip_id: tripId,
                      source: "trip_strip",
                    },
            ...
            {isInputSecurityRecoveryActive &&
              securityRecovery?.onOpenReview &&
              !securityRecovery.reviewOpen && (
                <button
                  type="button"
                  onClick={() => {
                    trackEvent("trip_generation__security_review--open", {
                      trip_id: tripId,
                      source: "trip_strip",
                    });
                    securityRecovery.onOpenReview?.();
                  ...
                  className="px-3 py-1 rounded-md bg-white text-xs font-semibold border border-current/20 hover:bg-white/80"
                  {...getAnalyticsDebugAttributes(
                    "trip_generation__security_review--open",
                    {
                      trip_id: tripId,
                      source: "trip_strip",
                    },
                ...
                >
                  {t("tripView.generation.securityRecovery.reviewAction")}
                </button>
              )}
            {generationState === "failed" &&
              !isPendingAuthGeneration &&
              ...
              onRetryGeneration && (
                <button
                  ...
                  className="px-3 py-1 rounded-md bg-white text-xs font-semibold border border-current/20 hover:bg-white/80 disabled:cursor-not-allowed disabled:opacity-50"
                  {...getAnalyticsDebugAttributes(
                    "trip_generation__trip_strip--retry",
                    {
                      trip_id: tripId,
                      source: "trip_strip",
                    },
      ...
      {isInputSecurityRecoveryActive && securityRecovery?.reviewOpen && (
        <div className="border-b border-amber-200 bg-amber-50 px-4 py-4 sm:px-6">
          <div className="rounded-2xl border border-amber-200 bg-white/90 p-4 shadow-sm">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <p className="text-sm font-semibold text-amber-950">
                  {t("tripView.generation.securityRecovery.cardTitle")}
                </p>
                <p className="text-sm leading-6 text-amber-900">
                  {t("tripView.generation.securityRecovery.cardDescription")}
                </p>
              </div>
              <span className="inline-flex w-fit rounded-full bg-amber-100 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.08em] text-amber-800">
                {t("tripView.generation.securityRecovery.flaggedBadge")}
              </span>
            </div>

            <div className="mt-4 grid gap-3">
              {securityRecovery.fields.map((field) => {
                const value =
                  field.key in securityRecovery.draft
                    ? securityRecovery.draft[field.key] || ""
                    : field.value;
                return (
                  <label
                    key={field.key}
                    className="block rounded-xl border border-amber-200 bg-amber-50/60 p-3"
                  >
                    <span className="mb-2 block text-xs font-semibold uppercase tracking-[0.08em] text-amber-800">
                      {field.label}
                    </span>
                    <textarea
                      value={value}
                      onChange={(event) =>
                        securityRecovery.onDraftChange?.(
                          field.key,
                          event.target.value,
                        )
                      }
                      rows={field.multiline ? 4 : 2}
                      className="w-full rounded-lg border border-amber-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm outline-none transition focus:border-amber-400 focus:ring-2 focus:ring-amber-200"
                    />
                  </label>
                );
              })}
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <button
                type="button"
                onClick={() => {
                  trackEvent("trip_generation__security_review--retry", {
                    trip_id: tripId,
                    source: "trip_strip",
                  });
                  securityRecovery.onRetryWithChanges?.();
                }}
                disabled={!securityRecovery.canRetry || isRetryingGeneration}
                className="rounded-md bg-amber-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-amber-700 disabled:cursor-not-allowed disabled:opacity-50"
                {...getAnalyticsDebugAttributes(
                  "trip_generation__security_review--retry",
                  {
                    trip_id: tripId,
                    source: "trip_strip",
                  },
              ...
              >
                {isRetryingGeneration
                  ? t("tripView.generation.strip.retrying")
                  : t("tripView.generation.securityRecovery.retryAction")}
              </button>
              <button
                type="button"
                onClick={() => {
                  trackEvent("trip_generation__security_review--clear", {
                    trip_id: tripId,
                    source: "trip_strip",
                  });
                  securityRecovery.onClearFlaggedText?.();
                }}
                className="rounded-md border border-amber-300 bg-white px-3 py-2 text-xs font-semibold text-amber-900 transition hover:bg-amber-100"
                {...getAnalyticsDebugAttributes(
                  "trip_generation__security_review--clear",
                  {
                    trip_id: tripId,
                    source: "trip_strip",
                  },
              ...
              >
                {t("tripView.generation.securityRecovery.clearAction")}
              </button>
              <button
                type="button"
                onClick={() => {
                  trackEvent("trip_generation__security_review--cancel", {
                    trip_id: tripId,
                    source: "trip_strip",
                  });
                  securityRecovery.onCancelReview?.();
                }}
                className="rounded-md border border-transparent px-3 py-2 text-xs font-semibold text-amber-900 transition hover:bg-amber-100"
                {...getAnalyticsDebugAttributes(
                  "trip_generation__security_review--cancel",
                  {
                    trip_id: tripId,
                    source: "trip_strip",
                  },
              ...
              >
                {t("tripView.generation.securityRecovery.cancelAction")}
              </button>
            </div>

            <p className="mt-3 text-xs text-amber-900/80">
              {securityRecovery.canRetry
                ? t("tripView.generation.securityRecovery.readyHint")
                : securityRecovery.hasChanges
                  ? t("tripView.generation.securityRecovery.requiredFieldHint")
                  : t("tripView.generation.securityRecovery.changeHint")}
            </p>
          </div>
        </div>
      )}
      ...
      {hasConflictBackupForTrip && onRestoreConflictBackup && (
        <div className="px-4 sm:px-6 py-2 border-b border-violet-200 bg-violet-50 text-violet-900 text-xs flex items-center justify-between gap-3">
          ...
          <button
            ...
            {...getAnalyticsDebugAttributes(
              "trip_connectivity__trip_strip--restore_backup",
              {
                trip_id: tripId,
              },
            )}
      ...
      {shareStatus && (
        <div className="px-4 sm:px-6 py-2 border-b border-amber-200 bg-amber-50 text-amber-900 text-xs flex items-center justify-between">
          <span>
            {shareStatus === "view"
              ? "View-only shared trip"
              : "Shared trip · Editing enabled"}
          </span>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="min-w-0 flex-1">
              <p className="font-semibold">
                {adminOverrideEnabled
                  ? "Admin override editing is enabled."
                  : "Admin fallback view is read-only by default."}
              </p>
              ...
              {!canEnableAdminOverride && (
                <p className="mt-1 text-[11px] text-indigo-800">
                  You do not have trip write permission, so editing cannot be
                  enabled.
                </p>
              )}
              {hasLoadingItems && (
                <p className="mt-1 text-[11px] text-indigo-800">
                  This trip has unfinished generation data, so some itinerary
            ...
            <div className="flex items-center gap-3">
              {ownerUsersUrl && (
                <button
                  ...
                  {...getAnalyticsDebugAttributes(
                    "trip_view__admin_owner--open_users",
                    {
                      trip_id: tripId,
                      owner_id: ownerId || null,
                    },
              ...
              <div className="inline-flex items-center gap-2">
                ...
                <label
                  ...
                  {...getAnalyticsDebugAttributes(
                    "trip_view__admin_override--toggle",
                    {
                      trip_id: tripId,
      ...
      {shouldShowExpirationBanner &&
        (tripExpiresAtMs || isTripLockedByExpiry) &&
        !isExampleTrip && (
          <div
            className={`border-b ${
              isTripLockedByExpiry
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <p className="min-w-0 text-sm font-semibold leading-6 text-current">
                      {isPaywallLocked
                        ? paywallRequiresLogin
                          ? expirationLabel
                            ? t("tripPaywall.strip.loginSinceDate", {
                                date: expirationLabel,
                              })
                            : t("tripPaywall.strip.loginNoDate")
                          : expirationLabel
                            ? t("tripPaywall.strip.directSinceDate", {
                                date: expirationLabel,
                              })
                    ...
                    {isPaywallLocked ? (
                      ...
                      >
                        {explorerTier.publicName} · $
                        {explorerTier.monthlyPriceUsd}
                        {t("shared.perMonth", { ns: "pricing" })}
                      </span>
                    ) : null}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 lg:justify-end">
                    {paywallStripUpgradeCheckoutPath && (
                      <Link
                        ...
                        onClick={() =>
                          trackEvent("trip_paywall__strip--upgrade", {
                            trip_id: tripId,
        ...
        <div className="fixed inset-x-3 bottom-[calc(env(safe-area-inset-bottom)+0.75rem)] z-[1450] sm:inset-x-auto sm:right-6 sm:bottom-6 sm:w-[420px]">
          <div className="rounded-2xl border border-accent-200 bg-white/95 px-4 py-3 shadow-xl backdrop-blur supports-[backdrop-filter]:bg-white/85">
            <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-accent-700">
              Example trip playground
            </p>
            <p className="mt-1 text-sm font-semibold text-slate-900">
              Explore freely. Copy when you want to keep and edit.
            </p>
            <p className="mt-1 text-xs leading-relaxed text-slate-600">
              ...
              {exampleTripBanner.countries.length > 0 && (
                <span>
                  {" "}
                  Country focus: {exampleTripBanner.countries.join(", ")}.
                </span>
              )}
            </p>
